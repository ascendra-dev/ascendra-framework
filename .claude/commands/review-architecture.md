...
5. Every file present in `projects/{PROJECT_CODE}/standards/` — the confirmed tech stack, architecture pattern, and every generated standard (cross-cutting, per-technology, and any additional pattern document). Do not assume a fixed list of filenames; read whatever is actually there. **Exception on a resumed session** (Section 3 below finds Section 12 already exists, `In Progress`): read `system-architecture.md` in full regardless — every concern's frame needs it — plus only the standards file(s) mapped to the concern about to be resumed: Concern A → `system-architecture.md` plus any architectural-pattern file; Concern B → `database-standards.md`; Concern C → `api-standards.md`; Concern D → `security-standards.md`; Concern E → `ci-cd-pipeline.md`, `environment-strategy.md`, `logging-standards.md`, `git-standards.md`. A concern already `Reviewed` does not need its mapped file reloaded this session — only load it again if reopening an `Open` flag against it. A fresh session (Section 12 does not exist yet) still reads every standards file, since every concern will be walked in order.
...
Check Section 12 of the architecture document.

---

**Fresh session (Section 12 does not exist):**

Append Section 12 to `projects/{PROJECT_CODE}/architecture/arch-v1.md` now:

```markdown
## Section 12 — Review Record
...
**Resumed session (Section 12 exists, status `In Progress`):**

Read Section 12. Find the first concern with status `Pending` or `In Progress`. That is the resume point.
...
**Review already complete (Section 12 exists, status `Complete`):**

> "The architecture review is already complete per Section 12. Current document status: `{status}`. Run `/update-status projects/{PROJECT_CODE}/architecture/arch-v1.md Locked` if it is not yet Locked."
...
Check the Open Flags table in Section 12. If any flags remain `Open`:
...
> "Architecture remains in Under Review status. Progress is saved in Section 12 — return to this review at any time. Run `/review-architecture {PROJECT_CODE}` to resume."
