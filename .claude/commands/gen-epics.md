...
3. **BRD — quick-reference first, if it exists:** `projects/{PROJECT_CODE}/brds/brd-core-v{N}-ref.md` (generated by `/review-brd` on approval, `FW-052`) carries every REQ verbatim plus personas, roles, and business rules — Section 2 (BRD Requirements Covered) needs REQ text word for word, and this is where it lives. **If it does not exist** (BRD approved before `FW-052`): read the full `brd-core-v*.md` instead (already verified in Step 1.5), state that explicitly, and recommend regenerating the ref file via `/review-brd`. Either way, open the full BRD only for content the ref doesn't carry (journey narrative, NFRs, a requirement's surrounding context) — state explicitly when you do. Read in full only if not already loaded this session.
