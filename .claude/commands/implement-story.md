...
1. Copy `../ascendra-ui/` (the whole repo) into a new `../{web-repo-name}/`, excluding `.git/`, `node_modules/`, `.next/`, `tsconfig.tsbuildinfo`, `package-lock.json`, `.DS_Store` — this is a filesystem copy from the local sibling, never a `git clone`, so it never pulls in `ascendra-ui`'s own git history/remote and never depends on network access.
