...
6. **UI component reference** (Web stories only) — check `projects/{PROJECT_CODE}/standards/` for a UI reference file (e.g. `ui-reference.md`). If present, scan it for the components relevant to this story before building any UI. If absent, use the UI library specified in the project's `system-architecture.md`.
   - **If the confirmed UI library is Ascendra UI:** also read `reference/ascendra-ui/hard-instructions.md` in full before building any UI. This file is a living list of corrections to how `ascendra-ui` is actually used in practice (found by PO review or direct investigation against `../ascendra-ui`'s real component source and real pages) — read *in addition to*, never instead of, `../ascendra-ui/docs/ui-reference.md`/`showcase-reference.md` (the sibling repo — these docs are never vendored into this project's own web repo, see Step 3's Web scaffolding). Where a hard instruction and a docs template disagree, the hard instruction wins — it exists specifically to record cases where the docs were wrong or incomplete.
...
**Web — if the confirmed frontend is Next.js + Ascendra UI (Section 2):** built on `ascendra-ui`'s component library, never scaffolded from scratch. `ascendra-ui` ships its own setup tool (`ascendra.js setup`) for exactly this — turning a full copy of the `ascendra-ui` repo into a blank consumer project — but **never** its own `ascendra.js update` for ongoing sync: `update` refuses to run without a TTY, always prompts interactively with no non-interactive escape hatch, and clones fresh from GitHub's default branch rather than the local sibling copy. Both would also duplicate or conflict with this framework's own project-management system (sprints, releases, PR descriptions, its own `.claude/commands`) if adopted wholesale. Do this instead:

**Choose and verify the vendoring source before copying anything — ask the PO:** local sibling copy, or fetch fresh from GitHub.
- **Local sibling (default — no network dependency):** run a preflight check in `../ascendra-ui/` first: `git status --porcelain` must be empty (no uncommitted or staged changes), the checked-out branch must be `main`, and `git rev-parse HEAD` must match `git rev-parse origin/main` (`git fetch origin` first so that comparison is current). If any of these fail, **stop and show the PO exactly what's dirty, stale, or off-branch** — the same stop-and-report pattern used above for a failed structural check. The PO then either fixes it (commit, stash, checkout `main`, pull) or explicitly confirms vendoring the tree as-is (e.g. deliberately testing an unmerged local change before it lands upstream). Only proceed to Step 1 below once the tree is clean or the PO has explicitly overridden.
- **Fresh from GitHub:** `git fetch origin` in `../ascendra-ui/` and copy from `origin/main` (or a PO-specified tag/branch) instead of the working tree — trading away the no-network-dependency property for a known-clean, reviewed source.

1. Copy the confirmed source (the local sibling `../ascendra-ui/`, once the preflight check above passes or the PO has overridden it, or the freshly fetched `origin/main`/tag if the PO chose GitHub) into a new `../{web-repo-name}/`, excluding `.git/`, `node_modules/`, `.next/`, `tsconfig.tsbuildinfo`, `package-lock.json`, `.DS_Store` — this is always a filesystem copy, never a `git clone`, so the result never pulls in `ascendra-ui`'s own git history/remote and the new repo's own history starts clean regardless of which source was used.
2. Run `node ascendra.js setup` inside `{web-repo-name}` (no arguments; it derives the project name from the folder). This strips `app/showcase/`, `scripts/` (doc-generation scripts), and `docs/` (showcase-only reference docs — `ascendra-ui` deliberately never ships these to a consumer; when this or a later story needs `ui-reference.md`/`showcase-reference.md`, read them from `../ascendra-ui/docs/` directly, the sibling repo, not from this project), resets `components/`, `hooks/`, `lib/`, `providers/`, `utils/` to empty (this project's own code goes here — **project-specific hooks go in `hooks/`, never nested under `lib/hooks/`**), and trims `package.json`'s `scripts` to `dev`/`build`/`start`/`lint`. It leaves the nested `ascendra-ui/` folder (the actual component library) and the root `app/{layout.tsx,globals.css,favicon.ico,page.tsx}` shell untouched — that library folder is managed: never hand-edit files inside it; refresh it wholesale when a sync is needed (see "Keeping Ascendra UI current" below).
3. `git init` in `{web-repo-name}`, default branch `main` — the copied tree has no `.git/` (excluded in Step 1), so this starts a clean, independent history, never `ascendra-ui`'s own.
4. Fix `eslint.config.mjs` — `setup` doesn't touch this file, so it still lints its own `ascendra-ui/` as first-party code: add `"ascendra-ui/**"` to `globalIgnores` (a vendored, never-hand-edited folder must not be linted — the copied config does not have this ignore, it must be added here).
5. Set up the test tooling `standards/test-strategy.md` specifies for this repo (e.g. Vitest config + setup file + test script) — neither `ascendra-ui` nor `setup` ships any test tooling, so this is always added fresh from the project's own test strategy.
6. Extend `package.json`: add whatever test-tooling `dependencies`/`devDependencies` and `scripts` entry Step 5 requires on top of what `setup` already carried over — `name`, the rest of the `dependencies`/`devDependencies` (from `ascendra-ui`'s own `package.json`), and `dev`/`build`/`start`/`lint` are already correct from Step 2.
7. Write `.gitignore` (standard Next.js — `node_modules/`, `.next/`, `*.tsbuildinfo`, `next-env.d.ts`, `.env*` — **with an explicit `!.env.example` exception**, otherwise the blanket `.env*` pattern silently untracks the one env file that must be committed) and `.env.example` from Section 7.3's frontend environment variables
8. **Generate the frontend auth session module now, before the API client depends on it (`FW-043`).** Per this project's confirmed auth provider and its auth-provider-specific standard (e.g. `supabase-auth-standards.md`) if one exists, otherwise `security-standards.md`'s frontend session/token-retrieval convention: generate `lib/auth.ts` (browser-only — session/token retrieval and any session-claims decoding used by Client Components, e.g. a `getBrowserAccessToken()`-style function) and, if the framework has a client/server execution split, a separate `lib/auth.server.ts` (Server Component/Route Handler session access) — split into two files specifically so a server-only import never leaks into a Client Component's bundle. This must exist before item 9 below, whose request interceptor calls directly into this module's token getter.
9. **Wire error handling now, as part of scaffolding — never left for whichever story happens to render the first screen (`FW-041`).** Per `standards/logging-standards.md`, `reference/api-contract/contract.md` (`FW-042`), and the relevant Tier 2 standard's error-handling rule:
...
10. Write `CLAUDE.md` and `README.md` from this project's actual context (`standards/system-architecture.md`, the domain glossary, `standards/git-standards.md`) — overwriting `setup`'s own generic versions. Cover what's genuinely load-bearing: `ascendra-ui/` is managed and must never be hand-edited, the `@/ascendra-ui` import convention, that this repo does **not** vendor `ascendra-ui`'s reference docs (a future dev extending this repo consults the `ascendra-ui` project directly for `ui-reference.md`/`showcase-reference.md`), and this project's actual branch/release model — not a generic feat/fix/chore workflow.
11. `npm install`
12. Commit: `chore: scaffold {web-repo-name} from ascendra-ui`

**Keeping Ascendra UI current:** no version manifest is tracked, so currency is determined by direct comparison, not a version number. When a story or `/assess-change` determines the project needs a newer `ascendra-ui` (a new/changed component, a shadcn primitive update): first apply the same source choice and preflight check as the initial scaffold above (ask the PO local vs. GitHub; if local, `../ascendra-ui/` must be clean, on `main`, and matching `origin/main` before it's trusted as the diff source — stop and report otherwise). Then diff `../ascendra-ui/ascendra-ui/` (or the freshly fetched `origin/main` if the PO chose GitHub) against `{web-repo-name}/ascendra-ui/` directly, and show the PO the diff before applying anything. On confirmation, re-copy the changed files wholesale (always safe — that tree is fully managed). Then diff the source's `package.json` dependencies against `{web-repo-name}/package.json` and flag any differences for the PO to approve before running `npm install`. Never touch `components/`, `hooks/`, `lib/`, `providers/`, `utils/`, or any file outside `ascendra-ui/` during a sync. (Deliberately not `ascendra.js update`: see the note above this list.)
...
- Check `../ascendra-ui/docs/ui-reference.md` (component API) and `../ascendra-ui/docs/showcase-reference.md` (page patterns) before writing any UI code — the sibling repo, never a copy inside this project — or `projects/{PROJECT_CODE}/standards/ui-reference.md` if this project maintains its own
