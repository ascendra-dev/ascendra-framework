...
Check the BRD for **Section 16 (Review Record)**.

**Fresh session (Section 16 does not exist):**

This is a new review. Create Section 16 at the end of the BRD with the following structure. Pre-populate the Requirement Log with every reviewable item from Sections 5, 6, 8, and 9, all set to `Pending`.
...
After creating Section 16, continue with the pre-walkthrough checks below before starting Step 4.
...
Note each open issue — bring it up in context during the walkthrough when the relevant requirement is reached. If a verification issue remains unresolved by the end of the walkthrough, add it to the Flags table in Section 16.
...
**Resumed session (Section 16 exists, status `In Progress`):**
...
**Review already complete (Section 16 exists, status `Complete`):**

> "The BRD review is already complete per Section 16. The BRD is ready for approval — type 'approved' to update the status, or raise any final concerns."
...
**After every outcome** (confirmed, changed, removed, or flagged), update the corresponding row in the Section 16 Requirement Log immediately. Do not wait until the end of the session.
...
| PO response | Action | Section 16 update |
|-------------|--------|-------------------|
...
For any rule challenged: apply the agreed change inline, confirm in one line, update the Section 16 row, continue.
...
For any integration challenged or removed: apply inline, confirm, update Section 16, continue.

If an integration has no corresponding security requirement in Section 9: add to the Section 16 Flags table — this must be resolved before approval.
...
For any requirement challenged: apply inline, confirm, update Section 16, continue.

Flag any integration from Section 8 that still has no corresponding security requirement — add to Section 16 Flags table.
...
After all sections are walked through, present the summary to the PO and update Section 16:
...
FLAGS — UNRESOLVED
[Numbered list of any items in the Section 16 Flags table still Open. If none: "None."]
...
Update the Section 16 Flags table to reflect current status of all flagged items.
...
Work through each remaining item. Apply agreed changes inline. Update Section 16 Flags table as each is resolved.
...
1. **Update Section 16 (Review Record):**
...
3. **Add a row to the Document Control table:**
   ```
   | {next version} | {today's date} | Ascendra AI | BRD review complete — see Section 16 for full review record. |
   ```
...
Status:     Approved ✅
Document:   {brd-path}
Record:     Section 16 — full review log retained in document
─────────────────────────────────────────
Next step:
