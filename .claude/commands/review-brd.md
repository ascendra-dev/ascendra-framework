...
4. **Generate the BRD quick-reference file.** Immediately after approval, write a companion file at `projects/{PROJECT_CODE}/brds/brd-core-v{N}-ref.md` (`FW-052`) — the same role `arch-v1-ref.md` plays for the architecture document. `/gen-epics` and `/gen-stories` read this instead of the full BRD for REQ-level planning, keeping their repeated reads lean as the project grows. Generating it here, at approval, rather than at initial drafting, means it always reflects whatever Section 17's walkthrough actually changed — never a pre-review snapshot. Regenerate it in full on every approval, including a re-approval after `/assess-change` bumps the version — never patch it in place.

   ```markdown
   # BRD Quick Reference — {PROJECT_CODE}
   **Generated from:** `brd-core-v{N}.md`  |  **Date:** {date}

   > Auto-generated companion to `brd-core-v{N}.md`. Do not edit manually.
   > `/gen-epics` and `/gen-stories` read this file first. Regenerated by `/review-brd` on every approval, and by `/assess-change` on every BRD version bump.

   ---

   ## Requirements
   | REQ ID | Description | Priority | Source | Layer | Walking Skeleton | Feature Area |
   |---|---|---|---|---|---|---|
   | REQ-001 | {verbatim from Section 5 — never paraphrased} | {Must Have / Should Have / Nice to Have} | {Client-Stated / Domain-Default / Assumed} | {Core / Ext:PK / Ext:School / Ext:[code]} | {Yes / —} | {Section 5.X heading this REQ sits under} |
   ...
   | Name | Summary |
   |---|---|
   | {persona or role name} | {one line, from Section 2 or Section 3} |
   ...
   | BR ID | Rule |
   |---|---|
   | BR-001 | {verbatim from Section 6} |

   ---

   For journey narrative, data requirements, integrations, security requirements, non-functional requirements, dependencies, and open questions: this file does not carry them — read `brd-core-v{N}.md` directly.
   ```
...
Status:     Approved ✅
Document:   {brd-path}
Quick ref:  projects/{PROJECT_CODE}/brds/brd-core-v{N}-ref.md
