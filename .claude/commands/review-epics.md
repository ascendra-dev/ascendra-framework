...
14. **Walking Skeleton Build Order** (FW-048) — Section 2's Walking Skeleton column matches the BRD's tagging for every REQ, exactly (`Yes`/`—`, never re-judged). If this epic carries any `Yes`-tagged REQ, it appears in `epics/index.md`'s Walking Skeleton Coverage table and is sequenced first (or tied-first with another Walking Skeleton epic) in the Dependency Graph build order — flag any epic carrying the tag that the build order places behind a non-Walking-Skeleton epic.
