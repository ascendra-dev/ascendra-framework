...
Work through the chapter's decision heuristics, density-guidance sections, and "Common mistakes" list, in the order they appear in the chapter. Some sections carry an italicized note directly under their heading explaining why they're not checkable here (a process decision made before this artifact existed, or a check that depends on a different file) — treat that note as settled and skip the section without re-deriving the judgment yourself. For every other section, apply it if it's genuinely checkable against this specific artifact's actual content (not every heuristic applies to every artifact even without an explicit note):
