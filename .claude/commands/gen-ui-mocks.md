...
7. `reference/ascendra-ui/hard-instructions.md` — the living registry of corrections to `ascendra-ui`'s own docs (cases where `ui-reference.md`/`showcase-reference.md` document a prop, wrapper, or pattern that doesn't actually match the real component source). Apply every entry that touches a component matched in this project's screens: build the mock from the real source, not from a documented-but-fictional pattern. This mock is what the PO approves and what `/implement-story` is later held to — a mock built on a wrong documented pattern would pass PO review here and then not match what implementation correctly builds, defeating the purpose of this phase (`FW-040`).
...
3. `reference/ascendra-ui/hard-instructions.md` — same reason as base mode; the gating captions and banners this pass adds are still real `ascendra-ui` markup and are subject to the same corrections.
4. **Component/reference source, only when Step 3a needs it (gating banner styling):** read the confirmed web repo name from architecture Section 3.1, then check whether `../{web-repo-name}` exists on disk.
