...
Extract:
- **PROJECT_CODE** — uppercase, hyphen-separated, with a trailing 3-digit sequence number. Format: `CLIENT-DOMAIN-NNN` (e.g. `HARBORVIEW-SCH-001`). The sequence number allows a PO to run multiple iterations of the same scope. This becomes the folder name.
  ...
  - **Structural validity:** if the code does not otherwise match `CLIENT-DOMAIN-NNN` (missing the 3-digit sequence number, no hyphens, extra segments), this is a real error — do not guess a correction. Stop and ask: "Project code must be in `CLIENT-DOMAIN-NNN` format (e.g. `HARBORVIEW-SCH-001`). You provided `{as-given}` — what should it be?"
