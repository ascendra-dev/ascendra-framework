...
3. **BRD — quick-reference first, if it exists:** `projects/{PROJECT_CODE}/brds/brd-core-v{N}-ref.md` (generated by `/review-brd` on approval, `FW-052`) carries every REQ verbatim plus personas, roles, and business rules — enough for the Step 4 planning table and Section 2 FR references. **If it does not exist** (BRD approved before `FW-052`): read the full `brd-core-v1.md` instead (look for the BRD path in the epic file if this default does not match), state that explicitly, and recommend regenerating the ref file via `/review-brd`. Either way, open the full BRD only for content the ref doesn't carry (journey narrative, NFRs, a requirement's surrounding context) — state explicitly when you do.
4. `[every domain knowledge file path referenced in the BRD]`

**Same-session reuse (items 3–4):** if the BRD (ref or full) or a domain knowledge file was already read earlier in this session — generating stories for a prior epic, say — do not re-read it. Reuse what's already in context, unless `/assess-change` has run on this project since.
