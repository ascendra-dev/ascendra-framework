...
**Never promote on trust alone — re-verify against current reality first.** A journal entry describes what was true when it was written; extraction can run gates later, and the implicated command, template, or convention may have already changed in between — the same "state is derived, never authoritative" posture this command already applies to its own reconciliation (§4.3). Before writing an observation, re-read the actual file the pattern points at and confirm the gap still exists. A pattern that's already been fixed since the entries were written is stale, not evidence.

**What it produces, at two different scales:**
- **Observations** (every extraction pass, and only for a pattern that's re-confirmed current) — the same `observations/` mechanism and promotion discipline as §8.5 already established (`New` → `Under Review` → `Promoted → FW-XXX` / `Rejected` / `Won't Fix`, `observations/index.md`, `observations/TEMPLATE.md`, `observations/OBS-NNN-{slug}.md`) is retained unchanged in shape — only its *input* and *timing* change. A candidate observation now has to show a real pattern across the accumulated entries (the same friction recurring, a limitation hit more than once) rather than being decided from a single in-the-moment incident, which is a stronger evidentiary bar than the original design could actually apply in practice.
...
| 2026-09-19 | Follow-up on §8.3: a direct question about whether a terse journal entry actually carries enough information for a much-later extraction pass surfaced a real gap — nothing told extraction to re-check a pattern against the implicated file's *current* state before promoting it, even though the framework's own file might have changed between the entry being written and extraction running. Fixed with one added safeguard, same "state is derived, never authoritative" posture §4.3 already applies elsewhere: extraction now re-reads the actual file a pattern points at and confirms the gap still exists before writing an observation — a pattern already fixed since the entry was written is stale, not evidence. `anchor-project.md` Step 11b and `decisions/FW-050` updated to match |
