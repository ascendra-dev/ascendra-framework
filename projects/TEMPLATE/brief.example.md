...
*Generated by `/judgment-check` against `practitioner-guide/01-intake.md` on 2026-06-08. This worked example demonstrates the section's shape — a real project's findings will differ.*

| # | Test (chapter source) | Location | Finding |
|---|---|---|---|
| 1 | Strategic Goals outcome-vs-feature test — "could the client achieve this goal without the system, in principle?" | Section 3, Goal 2 ("...that time drops to zero through automated dunning with no manual intervention required") | The goal names the mechanism ("automated dunning") rather than stopping at the outcome — does naming the mechanism here start to describe a feature the BRD should own instead of an outcome? |
| 2 | Constraints vs. Technology Preferences boundary — "if a constraint can be relaxed with a conversation, it is a preference — put it in Section 6" | Section 5 ("Other: Cloud hosting only...") vs. Section 6 ("Hosting: Cloud-managed on delivery team's infrastructure") | Cloud-vs-on-premise appears as a hard Constraint in Section 5 while a hosting line also appears in Section 6 — is this a genuine two-axis split (whether cloud at all vs. which cloud arrangement), or does Section 5's "Other" row duplicate what Section 6 already covers? |
| 3 | Risk vs. Open Question distinction — "an Open Question... without the answer, a requirement cannot be written" vs. "a Risk... needs a mitigation plan" | Section 8, row "DNS configuration for Resend email sending (SPF / DKIM records) — Open Question" | This item isn't something whose answer is needed to write a requirement — it's an operational readiness task with a real chance of delay (DNS propagation up to 72 hours) — does it more accurately belong as a Risk with a mitigation plan than as an Open Question? |
...
|------|---------|-------------|--------|
| 2026-06-08 | Goal 2 mechanism-naming | Confirmed as-is | "Automated dunning" isn't a specific implementation choice — it doesn't specify schedule, channel, or trigger logic; those genuinely feature-level decisions live in BRD REQ-017/018/019. Here it just distinguishes the class of solution from a non-system alternative (e.g. hiring more finance staff), which is exactly the alternative the goal-test asks about. The goal remains verifiable purely on the outcome ("time drops to zero"), independent of whichever specific dunning mechanism the BRD ultimately specifies. |
| 2026-06-08 | Section 5/6 hosting overlap | Confirmed as-is | Genuinely two different axes. Section 5's "cloud hosting only" is non-negotiable because Harborview has zero on-premise capability — no infrastructure, no IT staff — no conversation changes that. Section 6's "Cloud-managed on delivery team's infrastructure" is about which cloud arrangement and who manages it, both still open to the delivery team's judgment at architecture time. Section 5 rules out one entire option class; Section 6 covers what's still undecided within the option that remains. Not duplication. |
| 2026-06-08 | DNS row Type (Open Question vs. Risk) | Acknowledged tradeoff | Kept as Open Question deliberately. The brief template only carries two item types in Section 8 — a true "Dependency" tracking shape doesn't exist here; that's the BRD's Section 11 job. Since this item already has a named owner and a due date, and does need resolving before the invoice-email stories reach QA, Open Question is the closer structural fit even though it isn't phrased as a literal question. It gets re-tracked properly as a Dependency once the BRD exists — low cost either way since ownership and due date are already correctly captured here. |
