...
*Generated by `/judgment-check` against `practitioner-guide/08-development.md` on 2026-06-21. Findings are questions raised by the guide's own tests, not verdicts. This worked example demonstrates the section's shape — a real project's findings will differ.*

| # | Test (chapter source) | Location | Finding | Outcome | Detail |
|---|---|---|---|---|---|
| 1 | Two real template mismatches — Pre-Merge Checklist's `orgId`/React Query items hardcoded as unconditional (FW-029/FW-030) | Section 6 (Pre-Merge Checklist), items 6–7 | Both items are ticked "— N/A" here (migration only, no API endpoint; backend-only, no mutation hook). Is N/A the correct, honest application of Section 6's own AI Guide instruction for an inapplicable item — tick it and annotate, don't leave it unchecked — or does a migration-only PR call for these two items to be dropped from the checklist entirely rather than repeated as boilerplate N/A on every non-UI, non-API PR? | Confirmed as-is | This is exactly the treatment `pr-description.template.md` Section 6's own AI Guide already prescribes for an inapplicable item — tick it and add "— N/A," don't leave it unchecked and don't delete the line. Keeping both items on every PR's checklist (rather than removing them per-PR) gives reviewers one fixed list to scan the same way every time; the N/A annotation is what carries the stack-specific nuance FW-029/FW-030 require, not removing the line itself. No change. |
