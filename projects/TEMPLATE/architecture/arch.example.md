...
*Generated by `/judgment-check` against `practitioner-guide/06-architecture.md` on 2026-07-15. This worked example demonstrates the section's shape — a real project's findings will differ.*

| # | Test (chapter source) | Location | Finding |
|---|---|---|---|
| 1 | Actor Identity Shape — "ask the AI to name every actor kind it counted and why" | Section 2 / Actor Identity Shape derivation | Confirm every actor kind was genuinely enumerated — Finance Manager, Director, and Account Owner are all authenticated Staff; is there a fourth, non-Staff actor kind anywhere in this BRD? |
| 2 | Core/Extension methodology, applied directly — "apply the Membership Test yourself" | Section 3.2 (Extension Points — accounting integration) | Apply the Membership Test to the named extension point: "would every deployment of this base system need accounting integration, regardless of which extension is active?" |
| 3 | Deviation detection's blind spot — "the only real guardrail is attention in the moment" | Section 2 (Tech Stack table) | Every row matches the framework's common default with no deviation — was each row genuinely considered, or defaulted past without scrutiny? |
...
|------|---------|-------------|--------|
| 2026-07-15 | Actor Identity Shape completeness | Confirmed as-is | Finance Manager, Director, and Account Owner are all authenticated Staff — one identity space. The Payer only ever receives a Stripe-hosted payment link (BRD Journey 4.3); they never authenticate against or act within this system, so they're not an actor for this table's purposes. Single actor identity space, FK-based actor columns, correctly applied. |
| 2026-07-15 | Accounting integration extension point, Membership Test | Confirmed as-is | Fails the test deliberately, which is exactly why it's an extension point and not Core: not every deployment of this invoicing base needs accounting-system export — only clients who want one. Correctly placed as a named seam (Section 3.2), not built into Core. |
| 2026-07-15 | Tech Stack default-choice scrutiny | Confirmed as-is | Each row was checked against this project's actual signals (team size, budget posture, no compliance mandate ruling anything out) during Step 4/5 — matching the default wasn't a skip, it was the considered outcome. Recorded here so a future reviewer doesn't have to re-derive that this wasn't rubber-stamped. |
