...
> **[AI Guide]** Written only by `/judgment-check` — never filled at `/gen-architecture` time, and left out of the document entirely until that command has actually run once. This section holds every run's findings against `practitioner-guide/06-architecture.md`'s own tests (the Actor Identity Shape derivation, the deviation-detection blind spot, the composable-capability silent fork, and the rest of that chapter) — questions raised for the Product Owner to weigh, never verdicts. Each invocation either logs a new dated block of findings (only when nothing from a prior run is still open) or resolves whatever is still open (amending that finding's own row in place with the Product Owner's outcome — Confirmed as-is / Revised / Acknowledged tradeoff — never a new row for the same finding, never edited again once resolved). The section header itself is reserved here at generation time — always present — but its table is not; if `/judgment-check` has not yet run against this architecture document, leave the placeholder pointer line below exactly as it is rather than inventing a table.

> **[AI Guide — reserved numbering]** Section 11 is reserved for `/judgment-check` as shown here, always present from generation onward. `/review-architecture` creates its own **Section 12 (Review Record)** at the end of the document on first run — never Section 11, to avoid colliding with it. Section 12 follows immediately after Part 2 (Verification), whether or not `/judgment-check` has run yet — Section 11 is present either way, just with or without an actual table.
