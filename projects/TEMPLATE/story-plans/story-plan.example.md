...
*Generated by `/judgment-check` against `practitioner-guide/08-development.md` on 2026-07-05. This worked example demonstrates the section's shape — a real project's findings will differ. Note the numbering: this plan has no Worker Plan (Target is API+Web), so its content sections already end at 6 rather than 7 — Density & Judgment lands at 7/8 here, not the template's reserved 8/9, per the numbering note on `story-plan.template.md`.*

| # | Test (chapter source) | Location | Finding |
|---|---|---|---|
| 1 | Story-plan confirmation checklist — Auth/RBAC line | API-1 (Section 2), **Auth** line | `JwtAuthGuard`, `@Roles('finance_manager')` — does this match who you'd actually expect to create an invoice, not a plausible-looking placeholder? |
| 2 | Story-plan confirmation checklist — Section 6 (Risks / Open Questions) | Section 5 (Risks / Open Questions) | Section 5 reads "None." — but API-1's own **Audit / idempotency** line states "none required... creation is not a repeatable/idempotent-sensitive action." Is double-submission (network retry, double click on Save as Draft) genuinely nothing worth flagging, or does deciding that belong in Risks rather than being resolved silently inside the API block? |
| 3 | Two real template mismatches — Data Layer field's React Query wording | WEB-1 (Section 3), **Data layer** line | The line names a React Query hook (`useCreateInvoice`) that matches `story-plan.template.md`'s literal field wording exactly — is `useCreateInvoice` confirmed as this project's actual data-fetching mechanism per arch Section 2, or does it read this way because the template's hardcoded wording was followed rather than `gen-story-plan.md` Step 5's conditional instruction? |
...
|------|---------|-------------|--------|
| 2026-07-05 | API-1 Auth/RBAC line | Confirmed as-is | `JwtAuthGuard` + `@Roles('finance_manager')` is exactly the role arch Section 6.1 assigns to invoice creation, and matches the Finance Manager persona from the BRD. No change. |
| 2026-07-05 | Section 5 Risks = "None." | Confirmed as-is | Reference-number generation runs inside the same DB transaction as the invoice insert (API-1, Business logic step 4), so a genuine double-submit produces two distinct valid invoices with two distinct reference numbers — not a data-integrity problem. That's an accepted business outcome for manual invoice entry, not a defect worth a dedicated Risk line. |
| 2026-07-05 | WEB-1 Data layer / React Query wording | Acknowledged tradeoff | `useCreateInvoice` does match this project's confirmed frontend approach — arch Section 2 confirms React Query for Harborview — so kept as-is. Flagging that this field reading correctly here is a fact about this project's stack, not proof the template's hardcoded wording was followed deliberately; the next project confirming a hookless frontend has to actively override the template's literal label per `gen-story-plan.md` Step 5, since there's no worked hookless example anywhere in this file to copy from. |
