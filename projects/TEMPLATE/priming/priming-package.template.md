...
>
> `/run-priming-session` owns no subsection here — prose, an SRS, raw notes, and live conversation always have a home in Sections 4-6. `/run-priming-code-session` owns two: `Code Structure Reference` and `Sub-Domain Split Recommendation` — see `conventions/priming-command-conventions.md` PC-032 for what each captures and where each routes. Leave this section at its default ("None") when neither has contributed anything this session.
>
