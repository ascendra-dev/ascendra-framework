...
>
> **When writing this file (`/run-code-priming-session`):** map every business-facing fact extracted from a legacy codebase onto Sections 4-6, exactly like `/run-priming-session` does, per `conventions/priming-command-conventions.md` PC-020. Code-structure-only facts (API surface, schema-as-implemented) noted only incidentally while extracting — never pursued on purpose, per `PC-003` — go to this command's own two Section 10 subsections (Code Structure Reference; Sub-Domain Split Recommendation), registered in `priming-command-conventions.md` PC-032. Reuse the literal "PO stated:" field label from Sections 4-6 even for code-derived facts — that label names the field, not the fact's literal origin, the same convention `/run-priming-session` already applies to file-triaged content. Never read the full repository — see this command's own file for the entity-centric, checkpointed walk this implies.
>
...
>
> **More than one command can write into this file.** `/run-priming-session` maps everything it captures onto Sections 4-6 and owns no Section 10 subsection. `/run-code-priming-session` (reading a legacy codebase instead of prose) also maps onto Sections 4-6 first, but additionally owns two Section 10 subsections — see `conventions/priming-command-conventions.md`, which governs every priming-producing command uniformly, and its PC-032 registry for the full list.
...
>
> **A different case belongs here too: a fact that touches a topic which already has a home, but that home (Brief, Domain, or BRD) is already Approved/Locked.** This isn't "doesn't fit anywhere" — it's "out of scope for this session specifically because it's already settled," and it still must not be silently dropped (`conventions/priming-command-conventions.md` PC-015). Write "Where it probably belongs" as the affected artifact plus its status, e.g. "Domain — already Locked, needs `/assess-change`," so it's clear this routes to a formal change, not a live discovery question.
...
>
> `/run-priming-session` owns no subsection here — prose, an SRS, raw notes, and live conversation always have a home in Sections 4-6. `/run-code-priming-session` owns two: `Code Structure Reference` and `Sub-Domain Split Recommendation` — see `conventions/priming-command-conventions.md` PC-032 for what each captures and where each routes. Leave this section at its default ("None") when neither has contributed anything this session.
>
