...
>
> **A different case belongs here too: a fact that touches a topic which already has a home, but that home (Brief or Domain) is already Approved/Locked.** This isn't "doesn't fit anywhere" — it's "out of scope for this session specifically because it's already settled," and it still must not be silently dropped (`conventions/priming-command-conventions.md` PC-015). Write "Where it probably belongs" as the affected artifact plus its status, e.g. "Domain — already Locked, needs `/assess-change`," so it's clear this routes to a formal change, not a live discovery question.
