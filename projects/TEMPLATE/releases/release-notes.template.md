...
> **[AI Guide]** Written only by `/judgment-check` — never filled when these release notes are generated, and left out of the document entirely until that command has actually run once. This is the one artifact type in this framework where the section is kept fully dynamic rather than reserved-with-a-placeholder at generation time: release notes are explicitly client-facing (this template's own document-level rule bans story IDs, epic IDs, table names, API routes, and any other internal reference), so no line pointing at an internal PO tool is pre-created here the way it is in every other template. This section holds every run's findings against `practitioner-guide/10-release-support-hotfix.md`'s own tests (release-notes versioning by release vs. sprint, and the client-facing-language discipline that excludes story IDs, epic IDs, table names, and API routes) — questions raised for the Product Owner to weigh, never verdicts. Each invocation either logs a new dated block of findings (only when nothing from a prior run is still open) or resolves whatever is still open (amending that finding's own row in place with the Product Owner's outcome — Confirmed as-is / Revised / Acknowledged tradeoff — never a new row for the same finding, never edited again once resolved). If `/judgment-check` has not yet run against these release notes, this section does not appear in the document at all; do not pre-create it empty.
>
> **[AI Guide — numbering note]** Section 7 is reserved for `/judgment-check` as shown here, immediately after Section 6 (Change Summary) and before Part 2 (Pre-Release Verification) — note this template's Part 2 heading is named "Pre-Release Verification," not "Verification" as in every other template; that name is unchanged by this section. No `/review-release-notes` command exists in this framework, so there is no other command reserving a conflicting section number here.
