...
| UBR-007 | The invoice subtotal is the sum of all line item totals (quantity × unit price). VAT is applied to the subtotal, not to individual line items. The invoice total is subtotal + VAT amount. | Invoice calculation |
...
- [x] Section 10 (Known Variations) does not list any topic fixed by a UBR. VAT rate is a Known Variation (rate varies); UBR-007 fixes the calculation method (subtotal × rate) — not the rate itself. No conflict.
