...
*Generated by `/judgment-check` against `practitioner-guide/03-brd-discovery-and-scope-lock.md` on 2026-07-10. This worked example demonstrates the section's shape — a real project's findings will differ.*

| # | Test (chapter source) | Location | Finding |
|---|---|---|---|
| 1 | REQ-writing density — "one REQ = one independently testable capability" | REQ-002 (Payer dedup) | This REQ packs the dedup key, three key options, and a default value into one sentence — does the configurability genuinely belong in one REQ, or does the default value deserve its own line? |
| 2 | Section 5.21 vs. Section 15 test — "can you write one factual sentence backed by the domain doc or this session?" | REQ-022 (Xero export, Section 5.21) | Confirm this still passes the test now that Section 5.21 exists as a real section, not just illustrated in the guide |
| 3 | REQ density / Source tagging | REQ-016 (partial payment handling, tagged `[Assumed]`) | This is an inference, not a client-stated rule — is it still appropriate to leave as `[Assumed]` rather than routing to Section 12 (Open Questions) for client confirmation? |
...
|------|---------|-------------|--------|
| 2026-07-10 | REQ-002 density | Confirmed as-is | The configurability, the three key options, and the default aren't separate features you could ship independently — they're the specification of the one dedup-detection capability. Strip any one out and the requirement stops being buildable on its own (a developer would have to ask which key to check). One REQ is correct. |
| 2026-07-10 | REQ-022 Section 5.21 test | Confirmed as-is | The domain knowledge document already models "Accounting System" as a Typical Integration Point, and James Okafor specifically named Xero during discovery — both facts are on record, not guessed. Passes the test cleanly. |
| 2026-07-10 | REQ-016 `[Assumed]` tag | Acknowledged tradeoff | Kept as `[Assumed]` deliberately — the inference is low-risk (a sensible default for handling an underpayment) and re-confirming it with the client would cost a follow-up call for something reversible later at negligible cost if wrong. A higher-risk assumption would be routed to Section 12 instead; this one doesn't meet that bar. |
