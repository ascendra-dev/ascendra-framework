...
If you haven't yet moved an approved BRD through epic generation, that's the previous chapter's job — see `04-product-structuring-epics.md` (not yet written) for epic boundaries and sizing. This chapter picks up once every epic is `Approved`, which is `/gen-screen-design`'s own gate.
